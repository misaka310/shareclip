...
- 設定とアップロード履歴は Windows の `%APPDATA%\ShareClip` に保存され、リポジトリや配布zipには含まれません
...
初回起動時に OCI 設定が未入力なら、自動で `設定` 画面を開きます。設定が未完了の間はアップロードできず、アップロード画面からも `設定を開く` で戻れます。

`設定` 画面で次を入力し、`接続テスト` を押してから `設定を保存` を押します。
...
通常はアプリ画面で、ファイル選択またはドラッグ&ドロップ、接続テスト、アップロード、リンクのブラウザ確認、履歴からのコピー/再発行/削除を確認します。
...
1. `設定` 画面で OCI 情報を入力し、`接続テスト` が成功することを確認する
2. `アップロード` 画面でファイルを選ぶ、またはドラッグ&ドロップする
3. 期限を `1日` / `3日` / `7日` から選ぶ
4. `アップロードしてリンク生成` を押す
5. クリップボードに URL がコピーされる
6. ブラウザのシークレットウィンドウなどで URL を開き、ファイルがダウンロードできるか確認する
7. `共有リンク` の `コピー` で、同じ URL を後から再コピーでき、ボタン表示が一時的に `コピーしました` に変わることを確認する
8. `履歴` の `コピー` で、履歴から同じ URL を再コピーでき、ボタン表示が一時的に `コピーしました` に変わることを確認する
9. `履歴` の `1日` / `3日` / `7日` で、新しい期限の URL を再発行してコピーでき、`コピー` ボタン側の表示も一時的に `コピーしました` に変わることを確認する
10. `履歴` の `削除` で確認ダイアログが表示され、承認後に OCI 上のファイルを削除できることを確認する
...
署名付きURLは、有効期限内だけ使える一時的な秘密情報です。管理者にも読ませたくないファイルは、アップロード前に別途暗号化してください。
